...
import { Search, MoreHorizontal, Eye, UserCog, Users, ChevronLeft, ChevronRight, RefreshCw } from 'lucide-react';
...
import type { UserWithGemsCount, UserFilters } from '@/lib/api/admin-client';
...
export default function UsersPage() {
  ...
  const cacheKey = useMemo(() =>
    `admin-users-${page}-${role}-${country}-${search}`,
  ...
  const usersFetcher = useCallback(async () => {
    ...
    return response.json() as Promise<PaginatedResponse<UserWithGemsCount>>;
  }, [page, role, country, search]);
  ...
  const handleRoleChange = async (newRole: UserRole) => {
    ...
    try {
      ...
      cache.invalidatePrefix('admin-users');
      refetchUsers();
    } catch (error) {
